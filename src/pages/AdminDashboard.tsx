...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [profilesRes, rolesRes, medCount] = await Promise.all([
      ...
      setProfiles(profs);
      setMedicineCount(medCount.count || 0);
    };
    fetchData();
  ...
  const handleRemoveUser = async (userId: string) => {
    ...
    // Delete from auth via edge function would be needed for full delete
    // For now, remove profile
  ...
  const stats = [
    ...
    { icon: Pill, value: medicineCount, label: "Medicines", color: "bg-secondary text-secondary-foreground" },
  ];
                      ...
                      <motion.div key={u.user_id} layout exit={{ opacity: 0, x: -20 }}
                        ...
                        <Button variant={confirmDelete === u.user_id ? "destructive" : "ghost"} size="sm"
                          ...
                          {confirmDelete === u.user_id ? (
                            <span className="flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Confirm</span>
